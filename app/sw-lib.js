 ...
 * @memberof module:sw-runtime-caching
 */
const behaviorCallbacks = ['cacheDidUpdate', 'cacheWillMatch', 'cacheWillUpdate', 'fetchDidFail'];
...
class BaseCacheManager {
  ...
  getCachedUrls() {
    if (!this._cachedUrls) {
      this._cachedUrls = Array.from(this._entriesToCache.keys()).map(url => new URL(url, location).href);
    }
...
class PrecacheManager {
   ...
   */
  _performInstallStep() {
    return Promise.all([this._revisionedManager._performInstallStep(), this._unrevisionedManager._performInstallStep()]).then(() => {
  ...
  _performActivateStep() {
    return Promise.all([this._revisionedManager._cleanUpOldEntries(), this._unrevisionedManager._cleanUpOldEntries()]).then(() => {
      // Closed indexedDB now that we are done with the install step
      this._close();
    }).catch(err => {
      this._close();
